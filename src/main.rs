...
impl App {
    ...
    // Renders a frame of our Vulkan app.
...
unsafe fn create_instance(window: &Window, entry: &Entry, data: &mut AppData) -> Result<Instance> {
    let app_info = vk::ApplicationInfo::builder()
        .application_name(b"VK Tutorial\0")
        .application_version(vk::make_version(1, 0, 0))
        .engine_name(b"Memgine\0")
    ...
    if VALIDATION_ENABLED && !available_layers.contains(&VALIDATION_LAYER) {
        return Err(anyhow!("Validation layer requested by=ut not supported"));
    }
    ...
    let flags = InstanceCreateFlags::ENUMERATE_PORTABILITY_KHR;
...
unsafe fn check_physical_device(
    ...
    if features.tessellation_shader != vk::TRUE {
        return Err(anyhow!(SuitabilityError(
            "Missing tesselation shader support."
        )));
    }
...
impl QueueFamilyIndices {
    unsafe fn get(
        ...
        if let (Some(graphics), Some(present)) = (graphics, present) {
            ...
            Err(anyhow!(SuitabilityError(
                "Missing required queue fanilies."
            )))
        }
    }
}
...
unsafe fn create_swapchain(
    ...
    let mut image_count = support.capabilities.max_image_count + 1;
...
unsafe fn create_pipeline(device: &Device, data: &mut AppData) -> Result<()> {
    ...
    let vert_shader_module = create_shaer_module(device, &vert[..])?;
    let frag_shader_module = create_shaer_module(device, &frag[..])?;
...
unsafe fn create_shaer_module(device: &Device, bytecode: &[u8]) -> Result<vk::ShaderModule> {
...
unsafe fn create_render_pass(
    ...
    // Subpasses
    let color_attacment_ref = vk::AttachmentReference::builder()
        .attachment(0)
    ...
    let color_attachments = &[color_attacment_ref];
...
unsafe fn create_texture_image(
    ...
    data.mip_levels = (reader.info().width.max(reader.info().height) as f32).log2().floor() as u32 + 1;
...
unsafe fn get_depth_format(instance: &Instance, data: &AppData) -> Result<vk::Format> {
    let candidates = &[
        vk::Format::D32_SFLOAT,         // TODO: Select depth + stencil
        vk::Format::D32_SFLOAT_S8_UINT, // <- this one should be first
...
fn load_model(data: &mut AppData) -> Result<()> {
    ...
    let (models, _) = tobj::load_obj_buf(&mut reader, true, |_| {
        ...
        tobj::MTLLoadResult::Ok((vec![tobj::Material::empty()], map))
    })?;
...
unsafe fn generate_mipmaps(
    ...
    {
        return Err(anyhow!(
            "Texure image format does not support linear blitting!"
        ));
    }
